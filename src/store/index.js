...
const checkDictionaryNameConsistency = (state, name) => {
    if (name) {
        const duplicateDictionary = state.dictionaryList.find(dictionary => dictionary.name === name);
...
const checkDictionaryRowDataConsistency = (state, payload, excludeItself = false // from duplicate check
) => {
